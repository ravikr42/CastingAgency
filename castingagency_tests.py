...
class CastingAgencyTestCase(unittest.TestCase):
    ...
    def prepare_test_bed(self):
        for i in range(5):
            actor = Actor(name=fake.name(),
                          gender=app_utils.get_gender_char(),
    ...
    def test_add_movie_casting_assistant(self):
        response = self.client().post("/movies", json=self.movie,
                                      headers=self.assistant_heaer)
        data = json.loads(response.data)
    ...
    def test_add_actor_casting_director(self):
        response = self.client().post("/actors", json=self.actor,
                                      headers=self.director_header)
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
